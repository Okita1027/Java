...
/**
 * @author qzy
 * @time 2023/11/13 19:54 星期一
 * @title FloydWarshall多源最短路径算法
 */
public class FloydWarshall {
    ...
    static void floydWarshall(List<Vertex> graph) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            Map<Vertex, Integer> map =
                    v.edges.stream().collect(Collectors.toMap(e -> e.linked, e -> e.weight));
    ...
    static void print(Vertex[][] prev) {
        System.out.println("-------------------------");
        for (Vertex[] row : prev) {
            System.out.println(Arrays.stream(row).map(v -> v == null ? "null" :
                            v.name)
